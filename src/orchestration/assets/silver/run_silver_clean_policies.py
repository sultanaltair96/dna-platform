"""Silver clean policies asset."""

from dagster import AutomationCondition, asset

from src.core.silver_clean_policies import transform
from src.orchestration.utils import create_output_with_metadata


@asset(
    group_name="silver",
    description="Clean and validate policy data",
    compute_kind="polars",
    automation_condition=AutomationCondition.eager(),
    deps=["run_bronze_policies"],
)
def run_silver_clean_policies(run_bronze_policies):
    """Run silver policies transformation."""
    silver_path, df, metadata = transform()
    return create_output_with_metadata(silver_path, df, metadata)
